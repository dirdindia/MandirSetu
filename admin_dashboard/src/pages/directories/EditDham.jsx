...
export default function EditDham() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchDham = async () => {
      try {
        const response = await api.get(`/dhams/${id}`);
        const dham = response.data;
        setFormData({
          name: dham.name || '',
          ...
          state: dham.location?.state || '',
          pincode: dham.location?.pincode || '',
          latitude: dham.geolocation?.latitude || '',
          longitude: dham.geolocation?.longitude || '',
          phone: dham.contact?.phone || '',
          email: dham.contact?.email || '',
          website: dham.contact?.website || '',
          profilePic: dham.profilePic || '',
          gallery: dham.gallery || [],
          category: dham.category || '',
          schedule: dham.schedule || { openTime: '', closeTime: '' },
          howToReach: dham.howToReach || { bus: '', train: '', air: '' },
          bestTimeToVisit: dham.bestTimeToVisit || [],
          placesToVisitNear: dham.placesToVisitNear || [],
          majorFestivals: dham.majorFestivals || [],
          aartiTimings: dham.aartiTimings || [],
          visitorInformation: dham.visitorInformation || [],
          darshanTimings: dham.darshanTimings || [],
          mandirSetuTip: dham.mandirSetuTip || '',
          religiousImportance: dham.religiousImportance || '',
        });
      } catch (err) {
        console.error('Failed to fetch dham:', err);
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'Failed to load Dham details',
        });
      }
    };
    if (id) fetchDham();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.put(`/dhams/${id}`, formData);
      Swal.fire({
        icon: 'success',
        title: 'Dham Updated!',
