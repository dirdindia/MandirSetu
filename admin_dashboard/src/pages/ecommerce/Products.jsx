...
export default function Products() {
  ...
  const initialProductState = {
    ...
    sellingPrice: '', stock: 0, unit: 'piece', displayImage: '', gallery: [], isVisible: true, entityType: 'mandir', entityId: ''
  };
  ...
  useEffect(() => {
    if (isModalOpen) {
      const fetchEntities = async () => {
        try {
          const res = await axiosInstance.get(`/${newProduct.entityType}s?limit=100`);
  ...
  }, [isModalOpen, newProduct.entityType]);
  ...
  const openEditModal = (product) => {
    setNewProduct({ 
      ...
      entityType: product.mandir_id ? 'mandir' : (product.dham_id ? 'dham' : 'mandir'),
      entityId: product.mandir_id?._id || product.mandir_id || product.dham_id?._id || product.dham_id || ''
    });
  ...
  const handleSaveProduct = async (e) => {
    ...
    try {
      const payload = {
        ...
        displayImage: newProduct.displayImage,
        gallery: newProduct.gallery,
        isVisible: newProduct.isVisible
      };
      if (newProduct.entityType === 'mandir') payload.mandir_id = newProduct.entityId;
      else payload.dham_id = newProduct.entityId;
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-maroon-darker ">Products</h1>
          <p className="text-maroon-darker/60  text-sm mt-1">Manage physical products, prasad, and merchandise.</p>
        </div>
        <button 
            ...
            <tbody className="divide-y divide-slate-200 ">
              {isLoading ? (
                ...
                products.map((product) => (
                  <tr key={product._id} className="hover:bg-premium  transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        {product.displayImage ? (
                          <img src={product.displayImage} alt={product.name} className="w-12 h-12 rounded-lg object-cover border border-gold/20 " />
                        ) : (
                        ...
                        <div>
                          <p className="font-medium text-maroon-darker ">{product.name}</p>
                          {product.gallery?.length > 0 && (
                            <p className="text-xs text-maroon-darker/40">{product.gallery.length} extra images</p>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        {product.sellingPrice ? (
                          <>
                            <span className="font-semibold text-maroon-darker ">₹{product.sellingPrice} {product.unit && <span className="text-xs text-maroon-darker/60 font-normal">/ {product.unit}</span>}</span>
                            <span className="text-xs text-maroon-darker/40 line-through">₹{product.price}</span>
                          </>
                        ) : (
                          <span className="font-semibold text-maroon-darker ">₹{product.price} {product.unit && <span className="text-xs text-maroon-darker/60 font-normal">/ {product.unit}</span>}</span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-center">
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none cursor-pointer ${product.isVisible ? 'bg-maroon-darker' : 'bg-slate-300 '}`}
                      >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 flex items-start justify-center z-50 p-4 sm:p-6 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white  rounded-2xl shadow-xl w-full max-w-2xl border border-gold/20  my-8 sm:my-10 relative">
            <div className="flex items-center justify-between p-6 border-b border-gold/20  sticky top-0 bg-white  rounded-t-2xl z-10">
            ...
            <form onSubmit={handleSaveProduct} className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <select 
                    ...
                    onChange={(e) => setNewProduct({...newProduct, entityType: e.target.value, entityId: ''})}
                    className="w-full px-4 py-2.5 border border-gold/20 rounded-xl bg-premium text-maroon-darker focus:outline-none focus:ring-2 focus:ring-maroon"
                  >
                ...
                <div>
                  <label className="block text-sm font-medium text-maroon-darker/80 mb-1">Select {newProduct.entityType === 'mandir' ? 'Mandir' : 'Dham'} *</label>
                  <select 
                    required
                    value={newProduct.entityId}
                    onChange={(e) => setNewProduct({...newProduct, entityId: e.target.value})}
                    className="w-full px-4 py-2.5 border border-gold/20 rounded-xl bg-premium text-maroon-darker focus:outline-none focus:ring-2 focus:ring-maroon"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-maroon-darker/80  mb-1">Product Name *</label>
                  <input 
                    type="text" required
                ...
                <div>
                  <label className="block text-sm font-medium text-maroon-darker/80  mb-1">Category *</label>
                  <select 
                    required
                ...
                <div>
                  <label className="block text-sm font-medium text-maroon-darker/80  mb-1">Original Price (₹) *</label>
                  <input 
                ...
                <div>
                  <label className="block text-sm font-medium text-maroon-darker/80  mb-1">Selling Price (₹) <span className="text-maroon-darker/40 font-normal">(Optional)</span></label>
                  <input 
                    type="number" min="0"
                    placeholder="800"
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full px-4 py-2.5 border border-gold/20  rounded-xl bg-premium  text-maroon-darker  focus:outline-none focus:ring-2 focus:ring-maroon resize-none"
                />
              </div>

              {/* Display Image Upload */}
              <div className="p-4 rounded-xl border border-gold/20  bg-premium ">
                <label className="block text-sm font-bold text-maroon-darker  mb-3">Main Display Image</label>
                <div className="flex items-center gap-4">
                  <div className="w-24 h-24 rounded-xl bg-white  border border-gold/20  flex items-center justify-center overflow-hidden shrink-0">
                    {isUploadingDisplay ? (
                      ...
                      <ImageIcon size={28} className="text-maroon-darker/40" />
                    )}
                  </div>
                  <div className="flex-1">
                    <label className="cursor-pointer bg-white  border border-gold/20  hover:bg-premium  transition-colors text-maroon-darker/80  px-4 py-2 rounded-xl text-sm font-medium inline-flex items-center">
                    ...
                    <p className="text-xs text-maroon-darker/60 mt-2">Used as the main thumbnail. Recommended: 800x800px.</p>
                  </div>
                </div>
              </div>
              ...
              {/* Gallery Upload */}
              <div className="p-4 rounded-xl border border-gold/20  bg-premium ">
                <label className="block text-sm font-bold text-maroon-darker  mb-3">Gallery Images</label>
                <div className="space-y-4">
                  <div className="flex flex-wrap gap-3">
                    {newProduct.gallery.map((img, idx) => (
                      <div key={idx} className="relative w-20 h-20 rounded-lg overflow-hidden border border-gold/20  group">
                    ...
                    <label className="w-20 h-20 rounded-lg border-2 border-dashed border-gold/30  hover:border-blue-500 hover:bg-maroon/5  transition-colors flex flex-col items-center justify-center cursor-pointer text-maroon-darker/60">
                      {isUploadingGallery ? (
                        <Loader2 size={20} className="animate-spin text-maroon" />
                      ) : (
                        <>
                  ...
                  <p className="text-xs text-maroon-darker/60">Upload multiple images to show different angles of the product.</p>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <label className="block text-sm font-medium text-maroon-darker/80  mb-1">Current Stock *</label>
                  <input 
                    type="number" required min="0"
                ...
                <div>
                  <label className="block text-sm font-medium text-maroon-darker/80  mb-1">Unit *</label>
                  <select 
                    required
                ...
                <div className="flex items-center gap-3 pt-6">
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none cursor-pointer ${newProduct.isVisible ? 'bg-maroon-darker' : 'bg-slate-300 '}`}
                  >
                  ...
                  <div className="-mt-1">
                    <p className="text-sm font-medium text-maroon-darker/80 ">Visible on Store</p>
                    <p className="text-[10px] text-maroon-darker/60">Show to customers</p>
                  </div>
                </div>
              </div>

              <div className="pt-4 flex justify-end gap-3 border-t border-gold/20  sticky bottom-0 bg-white  rounded-b-2xl -mx-6 px-6 pb-2">
                <button 
                  type="button"
                  onClick={handleCloseModal}
                  className="px-4 py-2.5 rounded-xl text-sm font-medium text-maroon-darker/70  hover:bg-gold/10  transition-colors cursor-pointer"
                  disabled={isSaving}
                >
                  Cancel
                </button>
                <button 
                  ...
                  className="bg-gradient-to-r from-maroon-darker to-maroon-darker hover:from-blue-700 hover:to-indigo-700 text-white px-6 py-2.5 rounded-xl text-sm font-medium transition-colors flex items-center disabled:opacity-70 disabled:cursor-not-allowed cursor-pointer"
                >
