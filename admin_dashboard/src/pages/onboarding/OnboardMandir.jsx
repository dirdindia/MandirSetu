...
export default function OnboardMandir() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post('/mandirs', formData);
      Swal.fire({
        icon: 'success',
        title: 'Mandir Onboarded!',
        text: 'The Mandir has been successfully added to the platform.',
      ...
      setFormData({
        ...
        bestTimeToVisit: [], placesToVisitNear: [], majorFestivals: [], mandirSetuTip: '', religiousImportance: ''
      });
      setStep(1);
    } catch (err) {
      Swal.fire({
  ...
  return (
    <div className="max-w-4xl mx-auto py-6">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-maroon">Onboard New Mandir</h2>
        <p className="text-maroon-darker/60 mt-2">Fill out the details step-by-step to register a new temple.</p>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gold/20 overflow-hidden p-8">
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2">
                <label className="block text-sm font-bold text-maroon-darker/80 mb-2">Mandir Name *</label>
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-bold text-maroon-darker/80 mb-2">Mandir Affiliation *</label>
                <div className="flex gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="affiliationType" value="individual" checked={formData.affiliationType === 'individual'} onChange={handleChange} className="text-maroon focus:ring-maroon" />
                    <span className="text-maroon-darker/80">Individual Mandir</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="affiliationType" value="dham" checked={formData.affiliationType === 'dham'} onChange={handleChange} className="text-maroon focus:ring-maroon" />
                    <span className="text-maroon-darker/80">Under a Dham</span>
                  </label>
                </div>
              </div>
              ...
              {formData.affiliationType === 'dham' && (
                <div>
                  <label className="block text-sm font-bold text-maroon-darker/80 mb-2">Select Dham *</label>
                  <select name="dham" value={formData.dham} onChange={handleChange} className="w-full px-4 py-3 bg-premium border border-gold/30 rounded-xl focus:ring-2 focus:ring-maroon focus:outline-none">
              ...
              <div className="md:col-span-2">
                ...
                <textarea name="religiousImportance" rows="3" value={formData.religiousImportance} onChange={handleChange} className="w-full px-4 py-3 bg-premium border border-gold/30 rounded-xl focus:ring-2 focus:ring-maroon focus:outline-none resize-none" placeholder="Why is this temple religiously significant?"></textarea>
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm font-bold text-maroon-darker/80 mb-2">Mandir Setu Tip (Optional)</label>
                <input type="text" name="mandirSetuTip" value={formData.mandirSetuTip} onChange={handleChange} className="w-full px-4 py-3 bg-premium border border-gold/30 rounded-xl focus:ring-2 focus:ring-maroon focus:outline-none" placeholder="e.g. Visit early morning to avoid crowds." />
              </div>
              ...
              {['bestTimeToVisit', 'placesToVisitNear', 'majorFestivals', 'visitorInformation'].map((field) => (
                <div key={field} className="md:col-span-2 bg-premium p-4 rounded-xl border border-gold/20">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button type="button" onClick={() => handleAddArrayItem(field)} className="text-xs bg-gold/20 text-maroon px-3 py-1 rounded-full font-bold hover:bg-gold/30">
                      + Add Item
                    </button>
                  </div>
              ...
              {['aartiTimings', 'darshanTimings'].map((field) => (
                <div key={field} className="md:col-span-2 bg-premium p-4 rounded-xl border border-gold/20">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button type="button" onClick={() => handleAddTimingArrayItem(field)} className="text-xs bg-gold/20 text-maroon px-3 py-1 rounded-full font-bold hover:bg-gold/30">
                      + Add Item
                    </button>
                  </div>
                  {formData[field].map((val, idx) => (
        ...
        {step === 2 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="bg-gold/5 p-5 rounded-2xl border border-gold/20 mb-6">
              ...
              <div className="flex gap-3">
                <input 
                  ...
                  className="flex-1 px-4 py-3 bg-white border border-gold/20 rounded-xl focus:ring-2 focus:ring-maroon focus:outline-none" 
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="md:col-span-3 mt-4 p-5 bg-orange-50 dark:bg-orange-900/20 border border-gold/20 rounded-2xl">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h4 className="font-bold text-maroon flex items-center gap-2"><Map size={18}/> Geolocation (Required)</h4>
                    <p className="text-xs text-maroon dark:text-maroon mt-1">Fetch coordinates to show Mandir on the map.</p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-maroon/70 mb-1">Latitude</label>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-maroon/70 mb-1">Longitude</label>
        ...
        {step === 3 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="border-2 border-dashed border-gold/30 rounded-2xl p-6 text-center hover:bg-premium transition-colors">
              ...
              <p className="text-xs text-maroon-darker/60 mb-4">Upload the main image for the Mandir (JPEG, PNG)</p>
            ...
            <div className="border-2 border-dashed border-gold/30 rounded-2xl p-6 text-center hover:bg-premium transition-colors">
              ...
              <label htmlFor="gallery" className="cursor-pointer inline-flex items-center gap-2 px-4 py-2 bg-gold/20 text-maroon border border-gold/30 rounded-lg text-sm font-bold hover:bg-gold/30 transition-colors">
        ...
        {step === 4 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="mt-8 p-4 bg-orange-50 text-orange-800 rounded-xl text-sm font-medium border border-gold/30">
        ...
        <div className="flex justify-between mt-10 pt-6 border-t border-gold/10">
          ...
          {step < 4 ? (
            ...
            >
              {loading ? <><Loader2 size={18} className="animate-spin" /> Saving...</> : <><Save size={18} /> Submit Mandir</>}
            </button>
          )}
        </div>
      </div>
