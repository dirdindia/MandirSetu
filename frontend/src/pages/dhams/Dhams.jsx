      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-12 text-center">
          <h1 className="text-4xl sm:text-5xl font-serif font-bold text-maroon mb-4 drop-shadow-sm">
            Sacred <span className="text-gold">Dhams</span>
          </h1>
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {dhams.map((dham) => (
                <div key={dham._id} className="bg-white border border-gold/20 rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-maroon/10 transition-all duration-500 group flex flex-col">
                  <div className="h-56 overflow-hidden bg-premium relative shrink-0 border-b border-gold/10">
                    <img 
                      src={dham.profilePic || `https://ui-avatars.com/api/?name=${encodeURIComponent(dham.name)}&background=791916&color=fff`} 
                      alt={dham.name} 
                      className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"
                      onError={(e) => { e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(dham.name)}&background=791916&color=fff` }}
                    />
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    <h3 className="text-2xl font-serif font-bold text-maroon mb-2 truncate group-hover:text-maroon-darker transition-colors" title={dham.name}>{dham.name}</h3>
                    <div className="flex items-center text-maroon-darker/70 font-medium text-sm mb-4">
                      <span className="mr-1.5 text-gold">📍</span> {dham.location?.city || 'Unknown'}, {dham.location?.state || 'India'}
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-auto pt-5 border-t border-gold/20">
                      <div className="text-sm truncate mr-2">
                         ...
                         <span className="font-semibold text-maroon">{dham.mainDeity || 'N/A'}</span>
                      </div>
                      <Link to={`/dham/${dham._id}`} className="px-5 py-2.5 bg-gradient-to-r from-maroon to-maroon-dark text-premium hover:shadow-lg hover:shadow-maroon/30 rounded-xl text-sm font-bold transition-all shrink-0">
                        View Details
                      </Link>
                    </div>
                  </div>
