...
export default function AnyaDevtaHome() {
  ...
  const heroImages = ['/hero/img4.jpg', '/hero/img5.jpg'];
  const gradient = 'from-yellow-100 via-yellow-50 to-white dark:from-slate-900 dark:to-slate-800';
  const tilak = <div className="text-[10rem] sm:text-[14rem] md:text-[16rem] leading-none drop-shadow-2xl text-yellow-500">🕉️</div>;
  ...
  useEffect(() => {
    const fetchTemples = async () => {
      try {
        const res = await api.get('/mandirs?sect=anya');
        const data = res.data;
    ...
    const fetchDhams = async () => {
      try {
        const res = await api.get('/dhams?sect=anya');
        const data = res.data;
