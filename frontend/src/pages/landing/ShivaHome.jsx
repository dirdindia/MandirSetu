...
export default function ShivaHome() {
  ...
  const heroImages = ['/hero/img6.jpg', '/hero/img4.jpg'];
  const gradient = 'from-slate-200 via-slate-100 to-white dark:from-slate-900 dark:to-slate-950';
  const tilak = (
    <svg viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-64 sm:h-80 md:h-96 w-auto object-contain opacity-100 transition-all drop-shadow-2xl">
      <path d="M20 35 Q 50 45 80 35" stroke="#475569" strokeWidth="6" strokeLinecap="round"/>
      <path d="M20 50 Q 50 60 80 50" stroke="#475569" strokeWidth="6" strokeLinecap="round"/>
      <path d="M20 65 Q 50 75 80 65" stroke="#475569" strokeWidth="6" strokeLinecap="round"/>
  ...
  useEffect(() => {
    const fetchTemples = async () => {
      try {
        const res = await api.get('/mandirs?sect=shiva');
        const data = res.data;
    ...
    const fetchDhams = async () => {
      try {
        const res = await api.get('/dhams?sect=shiva');
        const data = res.data;
