...
export default function VaishnavaHome() {
  ...
  const gradient = 'from-orange-100 via-orange-50 to-white dark:from-slate-900 dark:to-slate-800';
  const tilak = <img src="/vaishnav-tilak.svg" alt="Vaishnava" className="relative h-64 sm:h-80 md:h-96 w-auto object-contain opacity-100 transition-all" />;
  ...
  useEffect(() => {
    const fetchTemples = async () => {
      try {
        const res = await api.get('/mandirs?sect=vaishnava');
        const data = res.data;
    ...
    const fetchDhams = async () => {
      try {
        const res = await api.get('/dhams?sect=vaishnava');
        const data = res.data;
    ...
    >
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-20 flex flex-col items-center w-full pt-10 pb-8">
        <h1 className="text-4xl sm:text-6xl md:text-7xl font-black tracking-tight text-slate-900 dark:text-white leading-tight">
          Discover Vaishnava's Sacred <br />
