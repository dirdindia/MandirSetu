    ...
    <div className="bg-premium min-h-screen font-sans pb-0 text-maroon-darker selection:bg-[#dfba6b]/30 overflow-x-hidden">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
            ...
            >
              ...
              <p className="text-[#dfba6b] text-base sm:text-lg max-w-2xl mx-auto lg:mx-0 leading-relaxed mb-10 font-light opacity-90">
                A sacred spiritual forum located in {temple.location?.city || 'India'}, renowned for its ancient origins and divine energy. Plan your darshan and experience inner peace.
              </p>
            ...
            >
              <div className="relative w-full max-w-md aspect-[3/4] rounded-3xl overflow-hidden border-4 border-[#dfba6b]/20 shadow-[0_0_50px_rgba(223,186,107,0.15)] group">
                <img 
                   src={temple.profilePic || `https://ui-avatars.com/api/?name=${encodeURIComponent(temple.name)}&background=5a1617&color=dfba6b&size=1024`} 
                   alt={temple.name} 
              ...
              <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 w-[85%] max-w-[320px] bg-white/95 backdrop-blur-md rounded-2xl p-4 shadow-2xl border-2 border-[#dfba6b] z-20">
                <div className="flex justify-between items-center text-center divide-x divide-[#dfba6b]/30">
                  <div className="flex-1 px-2">
                    ...
                    <p className="text-[#5a1617] font-serif font-bold text-sm">{temple.establishedYear || 'Ancient'}</p>
                  </div>
                  <div className="flex-1 px-2">
                    <p className="text-[10px] uppercase text-[#c09642] font-bold mb-1">Status</p>
                    <p className="text-[#5a1617] font-serif font-bold text-sm capitalize">{temple.status || 'Active'}</p>
                  </div>
                </div>
              </div>
            </motion.div>
      ...
      <section className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-16 mb-20">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{delay: 0.3}} className="bg-white rounded-2xl p-5 shadow-lg border border-[#dfba6b]/20 flex flex-col justify-center">
            ...
            <p className="text-[#5a1617] font-semibold text-sm sm:text-base leading-tight pl-8">{temple.mainDeity || 'N/A'}</p>
          </motion.div>
          ...
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{delay: 0.4}} className="bg-white rounded-2xl p-5 shadow-lg border border-[#dfba6b]/20 flex flex-col justify-center">
            ...
            <p className="text-[#5a1617] font-semibold text-sm sm:text-base leading-tight pl-8 truncate">{temple.location?.city ? `${temple.location.city}, ${temple.location.state}` : 'India'}</p>
          </motion.div>
          ...
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{delay: 0.5}} className="bg-white rounded-2xl p-5 shadow-lg border border-[#dfba6b]/20 flex flex-col justify-center">
            ...
            <p className="text-[#5a1617] font-semibold text-sm sm:text-base leading-tight pl-8">{temple.schedule?.openTime ? `${temple.schedule.openTime} - ${temple.schedule.closeTime}` : 'N/A'}</p>
          </motion.div>
          ...
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{delay: 0.6}} className="bg-white rounded-2xl p-5 shadow-lg border border-[#dfba6b]/20 flex flex-col justify-center">
            ...
            {temple.contact?.website ? (
              <a href={temple.contact.website} target="_blank" rel="noreferrer" className="text-[#5a1617] hover:text-[#dfba6b] font-semibold text-sm sm:text-base leading-tight pl-8 truncate">Visit Official Site</a>
            ) : (
          ...
          <div className="lg:col-span-1">
            ...
            >
              ...
              <div className="space-y-4 mb-6">
                {temple.darshanTimings?.length > 0 ? (
                  temple.darshanTimings.map((dt, idx) => (
                  ...
                  <>
                    <div className="flex justify-between items-center border-b border-dashed border-[#dfba6b]/40 pb-2">
                      ...
                      <span className="text-[#5a1617] font-bold">{temple.schedule?.openTime || 'N/A'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-dashed border-[#dfba6b]/40 pb-2">
                      <span className="text-[#5c4949] font-medium">Evening Darshan</span>
                      <span className="text-[#5a1617] font-bold">{temple.schedule?.closeTime || 'N/A'}</span>
                    </div>
                  </>
                )}
                
                {temple.aartiTimings?.length > 0 && temple.aartiTimings.map((at, idx) => (
                  <div key={`at-${idx}`} className={`flex justify-between items-center ${idx < temple.aartiTimings.length - 1 ? 'border-b border-dashed border-[#dfba6b]/40' : ''} pb-2`}>
      ...
      >
        <div className="text-center mb-8">
           ...
           <p className="text-[#c09642] text-xs sm:text-sm mt-3 uppercase tracking-wider font-semibold">Information regarding services and offerings available at the temple.</p>
        </div>
             ...
             <div className="space-y-6">
                ...
                {temple.majorFestivals?.length > 0 && (
                  <div>
                    ...
                    <ul className="space-y-2">
                      {temple.majorFestivals.map((fest, idx) => (
                        <li key={idx} className="flex items-start gap-2 text-[#5c4949] font-medium text-sm sm:text-base">
                          <div className="w-4 h-4 mt-0.5"><Star className="w-3 h-3 text-[#dfba6b]" /></div>
                          <span>{fest}</span>
                        </li>
             ...
             {temple.howToReach?.air && (
               <div className="bg-white rounded-2xl p-5 border border-[#dfba6b]/30 shadow-sm flex items-start gap-4">
                  ...
                  <div>
                     ...
                     <p className="text-[#5c4949] text-sm">{temple.howToReach.air}</p>
                  </div>
               </div>
             )}
             {temple.howToReach?.train && (
               <div className="bg-white rounded-2xl p-5 border border-[#dfba6b]/30 shadow-sm flex items-start gap-4">
                  ...
                  <div>
                     ...
                     <p className="text-[#5c4949] text-sm">{temple.howToReach.train}</p>
                  </div>
               </div>
             )}
             {temple.howToReach?.bus && (
               <div className="bg-white rounded-2xl p-5 border border-[#dfba6b]/30 shadow-sm flex items-start gap-4">
                  ...
                  <div>
                     ...
                     <p className="text-[#5c4949] text-sm">{temple.howToReach.bus}</p>
                  </div>
               </div>
             )}
          </motion.div>
      ...
      {temple.placesToVisitNear?.length > 0 && (
        <section className="w-full bg-[#5a1617] py-4 sm:py-6 mb-16 shadow-inner border-y border-[#dfba6b]/40 overflow-hidden">
          ...
          <div className="w-full flex overflow-hidden whitespace-nowrap">
            <div className="flex animate-marquee cursor-pointer w-max">
              {temple.placesToVisitNear.map((place, idx) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
             <div className="w-full h-full rounded-2xl overflow-hidden border border-[#dfba6b]/20 bg-[#fcf7ed]">
                {(() => {
                  let query = '';
                  if (temple.location?.coordinates && temple.location.coordinates.length >= 2) {
                    const lat = temple.location.coordinates[1];
                    const lng = temple.location.coordinates[0];
                    query = `${lat},${lng}`;
                  } else if (temple.location?.lat && temple.location?.lng) {
                    query = `${temple.location.lat},${temple.location.lng}`;
                  } else {
                    query = `${temple.name}, ${temple.location?.city || ''}`;
                  }
               ...
               <div className="space-y-4">
                 {temple.contact?.phone && (
                   <div className="flex items-center gap-4">
                     ...
                     <p className="text-[#5a1617] font-medium">{temple.contact.phone}</p>
                   </div>
                 )}
                 {temple.contact?.email && (
                   <div className="flex items-center gap-4">
                     <Mail className="w-5 h-5 text-[#c09642]" />
                     <p className="text-[#5a1617] font-medium">{temple.contact.email}</p>
                   </div>
                 )}
                 <div className="flex items-center gap-4">
                   <MapPin className="w-5 h-5 text-[#c09642] shrink-0" />
                   <p className="text-[#5a1617] font-medium text-sm">
                     {temple.location?.address ? temple.location.address : `${temple.location?.city || ''}, ${temple.location?.state || ''} ${temple.location?.zipCode || ''}`}
                   </p>
                 </div>
               </div>
             </div>
      ...
      {temple.gallery && temple.gallery.length > 0 && (
        <section className="w-full bg-[#fcf7ed] py-16 mb-24 mt-16 border-y border-[#dfba6b]/40 overflow-hidden shadow-inner" id="gallery">
          ...
          <div className="w-full flex overflow-hidden whitespace-nowrap">
            <div className="flex animate-marquee cursor-pointer w-max gap-6 px-3">
              {temple.gallery.map((img, idx) => (
                <div key={`g1-${idx}`} className="w-72 sm:w-96 h-64 rounded-2xl overflow-hidden shadow-lg border border-[#dfba6b]/40 group inline-block flex-shrink-0">
                   <img src={img} alt={`Gallery ${idx}`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                </div>
              ))}
              {/* Duplicate array for seamless infinite looping */}
              {temple.gallery.map((img, idx) => (
                <div key={`g2-${idx}`} className="w-72 sm:w-96 h-64 rounded-2xl overflow-hidden shadow-lg border border-[#dfba6b]/40 group inline-block flex-shrink-0">
                   <img src={img} alt={`Gallery ${idx}`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                </div>
              ))}
            </div>
