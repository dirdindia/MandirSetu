      ...
      <div class="text-left">
        ...
        <div class="space-y-6 px-2">
          ...
          <div class="grid grid-cols-2 gap-4 bg-premium  p-4 rounded-2xl border border-gold/20 ">
            <div>
              <p class="text-xs text-maroon-darker/60  uppercase font-bold tracking-wider mb-1">Founder / Guru</p>
              <p class="text-maroon-darker  font-bold text-sm">${ashram.founder || 'N/A'}</p>
            </div>
            <div>
              <p class="text-xs text-maroon-darker/60  uppercase font-bold tracking-wider mb-1">Capacity</p>
              <p class="text-maroon-darker  font-bold text-sm">${ashram.capacity ? ashram.capacity + ' pax' : 'N/A'}</p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <p class="text-maroon-darker  font-semibold text-sm">${ashram.contact?.managerName || 'N/A'}</p>
            </div>
            <div>
              <p class="text-xs text-maroon-darker/60  uppercase font-bold tracking-wider mb-1">Phone</p>
              <p class="text-maroon-darker  font-semibold text-sm">${ashram.contact?.phone || 'N/A'}</p>
            </div>
          </div>

          ${ashram.description ? `
          <div>
            <p class="text-xs text-maroon-darker/60  uppercase font-bold tracking-wider mb-2">About Ashram</p>
            <p class="text-maroon-darker/80  text-sm leading-relaxed">${ashram.description}</p>
          </div>` : ''}

          <div class="bg-orange-50/50  p-4 rounded-2xl border border-orange-100 ">
            <p class="text-xs text-orange-600  uppercase font-bold tracking-wider mb-2 flex items-center gap-2">
              Donations Accepted
            </p>
            <p class="text-maroon-darker  font-semibold text-sm">
              ${ashram.donationAccepted ? '<span class="text-green-600  flex items-center gap-1"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg> Yes, accepting donations</span>' : '<span class="text-maroon-darker/60">Not currently accepting</span>'}
            </p>
          </div>

          ${ashram.facilities && ashram.facilities.length > 0 ? `
          <div>
            <p class="text-xs text-maroon-darker/60  uppercase font-bold tracking-wider mb-2">Facilities Available</p>
            <div class="flex flex-wrap gap-2">
              ${ashram.facilities.map(f => `<span class="bg-orange-50  text-orange-700  px-3 py-1.5 rounded-lg font-semibold text-xs border border-orange-100  shadow-sm">${f}</span>`).join('')}
            </div>
          </div>` : ''}
          ...
          <div class="bg-red-50/50  p-4 rounded-2xl border border-red-100 ">
            ...
            <p class="text-maroon-darker/80  text-sm leading-relaxed">${ashram.rules}</p>
          </div>` : ''}
  ...
  return (
    <div className="max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-maroon-darker ">Ashrams Directory</h1>
          <p className="text-maroon-darker/70  mt-1">Manage and view all registered ashrams.</p>
        </div>
        <div className="relative w-full sm:w-64">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-maroon-darker/40" size={18} />
          <input type="text" placeholder="Search ashrams..." className="pl-10 pr-4 py-2 bg-white  border border-gold/20  text-maroon-darker  rounded-xl w-full focus:ring-2 focus:ring-orange-500 focus:outline-none transition-all shadow-sm" />
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-200 ">
              {loading ? (
                <tr>
                  <td colSpan="5" className="p-8 text-center text-maroon-darker/60 ">Loading ashrams...</td>
                </tr>
              ) : ashrams.length === 0 ? (
                <tr>
                ...
                ashrams.map((ashram) => (
                  <tr key={ashram._id} className="hover:bg-orange-50/50  transition-colors cursor-pointer" onClick={() => showDetails(ashram)}>
                    <td className="p-4 font-medium text-maroon-darker  flex items-center gap-3">
                      {ashram.profilePic ? (
                        <img src={ashram.profilePic} alt="" className="w-10 h-10 rounded-xl object-cover shadow-sm border border-gold/20 " />
                      ) : (
                      ...
                      <div>
                        ...
                        {ashram.associatedMandir && <p className="text-xs text-orange-600 ">{ashram.associatedMandir}</p>}
                      </div>
                    </td>
                    <td className="p-4 text-maroon-darker/70 ">
                      {ashram.location?.city}, {ashram.location?.state}
                    </td>
                    <td className="p-4 text-maroon-darker/70 ">{ashram.founder || 'N/A'}</td>
                    <td className="p-4 text-maroon-darker/70 ">{ashram.contact?.phone || 'N/A'}</td>
                    <td className="p-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <button 
                          onClick={(e) => { e.stopPropagation(); showDetails(ashram); }}
                          className="p-2 text-maroon-darker/60 hover:text-orange-600 hover:bg-orange-100  rounded-lg transition-colors inline-flex justify-center items-center"
                          title="View Details"
                        >
                          <Eye size={18} />
                        </button>
